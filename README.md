...
## 機械学習の発展
### 説明可能な AI
- AI を Business に適用するうえで、利用者が納得できる予測根拠を説明することは非常に重要。
- 機械学習 Model を開発する上で必須の知識になってくる。
- 利用者が納得できる予測根拠を説明できることは非常に重要なため、学習済み Model を差し替えて試行錯誤して、使いこなせるようになることで協力な Tool になる。
- BI(Business intelligence) Tool と相性がよく、予測値、説明変数、SHAP値をあわせて Dash-board化することで、Python が使えない利用者の向上に繋がる。
- あくまでも学習済み Model の振る舞いを解釈しているため、Model自体の精度が良くない場合は予期せぬ結果が出力される。Model の精度を最適化して活用すべし。
